...
/// <summary>
/// 抽牌堆专属浏览面板（零报错+防拉伸最终版）
/// </summary>
public partial class DrawPileBrowserPanel : Node2D
{
    ...
    #region 抽牌堆逻辑（零报错+防拉伸）
    private void ShowDrawCards()
    {
        ClearCards();

        if (DrawPileUI.Instance == null)
        {
            GD.PrintErr("❌ 抽牌堆UI未找到！");
            return;
        }

        List<CardData> cardDatas = DrawPileUI.Instance.GetAllCardDatas();
...
foreach (var data in cardDatas)
{
    ...
    try
    {
        ...
        // 【修改点】标记为抽牌堆卡牌，禁止打出
        card.CurrentZone = Card.CardZone.DrawPile;
